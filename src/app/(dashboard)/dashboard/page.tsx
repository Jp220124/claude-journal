...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-full max-w-[1200px] mx-auto p-6 md:p-10 flex flex-col gap-8">
      {/* Header Section */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-slate-500 dark:text-zinc-400 mt-1 text-base">
            {pendingTasksCount > 0 ? (
              <>You have <span className="text-slate-900 dark:text-white font-bold">{pendingTasksCount} pending tasks</span> today.</>
            ) : (
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            className="w-full bg-white dark:bg-zinc-800 text-slate-900 dark:text-white pl-10 pr-4 py-3 rounded-xl border border-slate-200 dark:border-zinc-700 focus:border-cyan-600 focus:ring-1 focus:ring-cyan-600 focus:outline-none transition-all placeholder-gray-400 dark:placeholder-zinc-500 text-sm shadow-sm"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Link
          href="/today"
          className="col-span-1 group flex flex-col items-start justify-between gap-4 p-5 rounded-2xl bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 hover:border-cyan-600 hover:ring-1 hover:ring-cyan-200 text-slate-900 dark:text-white transition-all cursor-pointer h-32 shadow-sm hover:shadow-md"
        >
          <div className="p-2 bg-slate-100 dark:bg-zinc-700 rounded-lg group-hover:bg-cyan-50 dark:group-hover:bg-cyan-900/30 group-hover:text-cyan-600 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pb-10">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          <div className="flex items-center justify-between">
            ...
            {hasReflection && (
              <button className="text-slate-500 dark:text-zinc-400 hover:text-cyan-600 text-sm font-medium transition-colors">
                View All
              </button>
            )}
          </div>
          ...
          {hasReflection ? (
            <div className="flex-1 bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-2xl p-6 md:p-8 relative overflow-hidden group shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="relative z-10 flex flex-col h-full">
                <div className="flex items-center gap-3 text-sm text-slate-500 dark:text-zinc-400 mb-4">
                  <span className="px-2 py-1 bg-slate-100 dark:bg-zinc-700 rounded text-xs font-mono text-slate-600 dark:text-zinc-300">
                ...
                <div className="prose prose-slate dark:prose-invert max-w-none mb-6">
                  ...
                  {isDemo && (
                    <p className="text-slate-400 dark:text-zinc-500 leading-relaxed mt-4">
                      {demoDashboardData.todaysReflection.preview}
                    </p>
                  )}
                </div>
                <div className="mt-auto pt-6 border-t border-slate-200 dark:border-zinc-700 flex gap-4">
                  <Link
                    href="/journal"
                  ...
                  <button className="flex items-center gap-2 text-slate-500 dark:text-zinc-400 font-medium text-sm hover:text-slate-900 dark:hover:text-white transition-colors">
            ...
            <div className="flex-1 bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 border-dashed rounded-2xl p-8 flex flex-col items-center justify-center text-center min-h-[300px]">
              <div className="w-16 h-16 bg-slate-100 dark:bg-zinc-700 rounded-full flex items-center justify-center mb-4">
                <span className="material-symbols-outlined text-slate-400 dark:text-zinc-500" style={{ fontSize: '32px' }}>edit_note</span>
              </div>
              <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">No entries yet</h3>
              <p className="text-slate-500 dark:text-zinc-400 mb-6 max-w-sm">Start your journaling journey by creating your first entry.</p>
              <Link
                href="/journal"
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          ...
          <div className="bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-2xl overflow-hidden flex flex-col h-full min-h-[400px] shadow-sm">
            {/* Progress Bar */}
            <div className="p-5 border-b border-slate-200 dark:border-zinc-700 bg-slate-50 dark:bg-zinc-800/50">
              <div className="flex justify-between items-end mb-2">
                <span className="text-sm font-medium text-slate-600 dark:text-zinc-400">Daily Progress</span>
            ...
            {tasks.length > 0 ? (
              <div className="flex flex-col divide-y divide-slate-200 dark:divide-zinc-700">
                {tasks.map((task) => (
                  <div
                    key={task.id}
                    className={`group p-4 flex gap-3 hover:bg-slate-50 dark:hover:bg-zinc-700/50 transition-colors cursor-pointer ${
                      task.completed ? 'bg-slate-50/50 dark:bg-zinc-800/50 opacity-70' : ''
                    }`}
                  >
                    <div className="relative flex items-start pt-1">
                      <input
                        ...
                        className={`peer appearance-none size-5 border-2 border-slate-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-800 cursor-pointer transition-all ${
                    ...
                    <div className="flex-1">
                      <p className={`text-sm font-medium leading-tight ${
                        task.completed ? 'text-slate-500 dark:text-zinc-500 line-through' : 'text-slate-900 dark:text-white group-hover:text-cyan-600 transition-colors'
                      }`}>
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        {!task.completed && task.priority && (
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${
                            task.priority === 'high' ? 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 border-red-100 dark:border-red-800' :
                            task.priority === 'medium' ? 'bg-orange-50 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 border-orange-100 dark:border-orange-800' :
                            'bg-slate-100 dark:bg-zinc-700 text-slate-500 dark:text-zinc-400 border-slate-200 dark:border-zinc-600'
                          }`}>
                            {task.priority}
                          </span>
                        )}
                        {task.completed && task.completedAt && (
                          <span className="text-xs text-slate-400 dark:text-zinc-500">Completed {task.completedAt}</span>
                        )}
                        {!task.completed && task.dueTime && (
                          <span className="text-xs text-slate-500 dark:text-zinc-400">Due {task.dueTime}</span>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                <div className="w-14 h-14 bg-slate-100 dark:bg-zinc-700 rounded-full flex items-center justify-center mb-4">
                  <span className="material-symbols-outlined text-slate-400 dark:text-zinc-500" style={{ fontSize: '28px' }}>task_alt</span>
                </div>
                <h3 className="text-base font-bold text-slate-900 dark:text-white mb-2">No tasks yet</h3>
                <p className="text-slate-500 dark:text-zinc-400 text-sm mb-4">Add your first task to get started</p>
                <Link
                  href="/today"
